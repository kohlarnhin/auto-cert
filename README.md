...
auto-cert/
  worker/                 # Cloudflare Worker 后端
    ...
    scripts/              # 部署时按环境变量生成 Wrangler 配置
...
把 `wrangler d1 create` 输出的 `database_id` 配置到 Cloudflare Builds 环境变量：

```text
CLOUDFLARE_D1_DATABASE_ID=your-d1-database-id
```

部署时会自动生成临时的 `worker/wrangler.toml`：

```toml
[[d1_databases]]
binding = "DB"
database_name = "auto-cert"
database_id = "来自 CLOUDFLARE_D1_DATABASE_ID"
```

如果 R2 bucket 名不是 `auto-cert`，配置可选环境变量：
...
[[r2_buckets]]
binding = "CERT_BUCKET"
bucket_name = "来自 CLOUDFLARE_R2_BUCKET_NAME"
```

## Worker 配置

仓库不提交真实 `worker/wrangler.toml`，也不需要 `.dev.vars`。部署脚本会读取环境变量生成 Wrangler 配置。
...
```text
WORKER_NAME=auto-cert-api
CLOUDFLARE_D1_DATABASE_NAME=auto-cert
CLOUDFLARE_R2_BUCKET_NAME=auto-cert
R2_KEY_PREFIX=auto-cert
R2_PUBLIC_BASE_URL=https://certs.example.com
CORS_ORIGINS=*
CERTIFICATE_WORKFLOW_NAME=auto-cert-certificate-workflow
```
...
`ENCRYPTION_KEY` 不写入 Git。部署脚本会从环境变量生成临时 secrets 文件，并通过 Wrangler 发布为 Worker Secret。生成方式：
...
Worker 运行时会自动执行 `CREATE TABLE IF NOT EXISTS` 初始化表结构，不会删除已有数据。你只需要提前创建 D1 数据库资源，并配置 `CLOUDFLARE_D1_DATABASE_ID`。
...
`worker/wrangler.toml` 不提交到 GitHub。部署脚本会读取 Cloudflare Builds 环境变量生成临时配置，避免 Wrangler 自动识别到 `frontend` 目录。
